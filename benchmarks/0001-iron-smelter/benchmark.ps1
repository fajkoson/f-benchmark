...
[string[]]$maps = dir -n -file | select-string $pattern

#path to factorio.exe
Set-Alias -Name factorio.exe -Value "d:\gamez\steamapps\common\Factorio\bin\x64\factorio.exe"
...
while ($i -lt $runs)
{
	...
	for ($k = 0; $k -lt $maps.length; $k++)
	{
		...
		echo "$map_name,$run_index,$startup_time,$end_time,$avg_ms,$min_ms,$max_ms,$ticks,$execution_time,$effective_UPS,$factorio_version,$platform" >> test_results_psscript.csv
	}
